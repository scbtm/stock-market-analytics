...
def mock_joblib():
    ...
    with patch(
        ...
        mock_model.named_steps = {"transformations": transforms, "model": regressor}
        ...
        for env_vars, expected_model in test_cases:
            with patch.dict(os.environ, env_vars, clear=True):
                ...
                expected_artifact_path = (
                    f"san-cbtm/stock-market-analytics/{expected_model}"
                )
                mock_wandb_api.artifact.assert_called_with(
                    expected_artifact_path, type="model"
                )
...
class TestInferenceDataIntegration:
    ...
    @patch(
        "stock_market_analytics.inference.inference_steps.generate_inference_features"
    )
    ...
    @patch("stock_market_analytics.inference.inference_steps.collect_inference_data")
    def test_get_inference_data_error_handling_integration(
        self, mock_collect, mock_generate
    ):
    ...
    @patch("stock_market_analytics.inference.inference_steps.collect_inference_data")
    def test_get_inference_data_empty_data_integration(
        self, mock_collect, mock_generate
    ):
...
class TestPredictionIntegration:
    ...
    def test_make_prediction_intervals_workflow_integration(self, mock_config):
        ...
        np.testing.assert_array_equal(
            result["pred_low_quantile"].values, [0.1, 0.2, 0.15]
        )
        np.testing.assert_array_equal(
            result["pred_high_quantile"].values, [0.9, 0.8, 0.85]
        )
    ...
    def test_predict_quantiles_pipeline_integration(self, mock_config):
        ...
        regressor.predict.assert_called_once_with(
            transforms.transform.return_value, return_full_quantiles=True
        )
