...
class TestVolatilityIndicators:
    ...
    def test_creates_volatility_indicators(self):
        ...
        assert "vol_ratio" in result.columns
        assert len(result.columns) == 5  # symbol, date, vol_ratio, long_vol_ewm, short_vol_ewm
    ...
    def test_selects_only_required_columns(self):
        ...
        expected_columns = {"symbol", "date", "vol_ratio", "long_vol_ewm", "short_vol_ewm"}
...
class TestMomentumIndicators:
    ...
    def test_creates_momentum_indicators(self):
        ...
        data = pl.DataFrame({
            ...
            "log_returns_d": [0.01, -0.005, 0.02, -0.01, 0.015, -0.02, 0.01, 0.005, -0.015, 0.02],
        })
...
class TestMomentumFeatures:
    ...
    def test_selects_correct_columns(self):
        ...
        # Should only contain the 5 specified columns
        expected_columns = {"symbol", "date", "long_short_momentum", "cmo", "risk_adj_momentum"}
        assert set(result.columns) == expected_columns
...
class TestLiquidityIndicators:
    ...
    def test_amihud_illiquidity_calculation(self):
        ...
        data = pl.DataFrame({
            ...
            "dollar_volume": [1000000, 2000000, 1500000],  # Higher volume should mean lower illiquidity
        })
...
class TestStatisticalIndicators:
    ...
    def test_creates_statistical_indicators(self):
        ...
        assert "log_returns_d" in result.columns
        assert len(result.columns) == 6  # symbol, date, kurtosis_ratio, skew_ratio, zscore_ratio, log_returns_d
...
class TestStatisticalFeatures:
    ...
    def test_creates_autocorr_and_iqr_features(self):
        ...
        data = pl.DataFrame({
            ...
            "skew_ratio": [0.0] * 20, 
            "zscore_ratio": [1.0] * 20,
            "log_returns_d": [0.01, -0.005, 0.02, -0.01, 0.015, -0.02, 0.01, 0.005, -0.015, 0.02] * 2,
        })
        ...
        assert len(result.columns) == 7  # symbol, date, kurtosis_ratio, skew_ratio, zscore_ratio, autocorr_r2, iqr_vol
...
class TestIchimokuIndicators:
    ...
    def test_creates_ichimoku_indicators(self):
        ...
        ichimoku_params = {
            "p1": 9, "p2": 26, "p3": 52, "atr_n": 14,
            "slope_window": 5, "persist_window": 10
        }
        ...
        # Check that expected columns are present  
        expected_cols = {
            "symbol", "date", "close", "tenkan", "kijun", "span_a_now", "span_b_now",
            "cloud_top_now", "cloud_bot_now", "atr", "price_above_cloud_atr", 
            "price_below_cloud_atr", "tenkan_kijun_spread_atr", "cloud_thickness_atr",
            "price_vs_lead_top_atr", "price_vs_lead_bot_atr"
        }
    ...
    def test_ichimoku_line_calculations(self):
        ...
        ichimoku_params = {"p1": 5, "p2": 10, "p3": 15, "atr_n": 5, "slope_window": 3, "persist_window": 5}
...
class TestIchimokuFeatures:
    ...
    def test_creates_ichimoku_features(self):
        ...
        expected_features = {
            "tenkan_slope", "kijun_slope", "above_cloud", "below_cloud",
            "tenkan_cross_up", "tenkan_cross_dn", "price_break_up", "price_break_dn",
            "twist_event", "twist_recent", "bull_strength", "bear_strength"
        }
...
class TestDfFeatures:
    ...
    def test_joins_all_feature_dataframes(self):
        ...
        result = df_features(
            basic_data, volatility_data, momentum_data, 
            liquidity_data, statistical_data, ichimoku_data
        )
