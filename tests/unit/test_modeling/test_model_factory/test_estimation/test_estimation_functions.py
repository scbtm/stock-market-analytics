...
class TestStandardizeData:
    ...
    def test_standardize_data_categorical_conversion(self):
        ...
        input_data = pd.DataFrame({
            ...
            'mixed_data': ['red', 'blue', '123'],  # Mix of strings and string numbers
            'special_chars': ['@', '#', '$']
        })
    ...
    def test_standardize_data_scientific_notation(self):
        ...
        input_data = pd.DataFrame({
            'scientific': ['1e2', '2.5e-1', '3.0e+3']
        })

        result = standardize_data(input_data)

        assert pd.api.types.is_numeric_dtype(result['scientific'])
        np.testing.assert_array_almost_equal(result['scientific'], [100.0, 0.25, 3000.0])
...
class TestCreateCatboostPool:
    ...
    def test_create_catboost_pool_catboost_available(self, sample_dataframe, sample_target):
    ...
    @patch('stock_market_analytics.modeling.model_factory.estimation.estimation_functions.CATBOOST_AVAILABLE', False)
    ...
    def test_create_catboost_pool_calls_standardize_data(self, sample_dataframe, sample_target):
        """Test that standardize_data is called."""
        if not CATBOOST_AVAILABLE:
            pytest.skip("CatBoost not available")

        with patch('stock_market_analytics.modeling.model_factory.estimation.estimation_functions.standardize_data') as mock_standardize:
    ...
    def test_create_catboost_pool_calls_detect_categorical(self, sample_dataframe, sample_target):
        """Test that detect_categorical_features is called."""
        if not CATBOOST_AVAILABLE:
            pytest.skip("CatBoost not available")

        with patch('stock_market_analytics.modeling.model_factory.estimation.estimation_functions.detect_categorical_features') as mock_detect:
    ...
    @patch('stock_market_analytics.modeling.model_factory.estimation.estimation_functions.Pool')
    def test_create_catboost_pool_with_categorical_features(self, mock_pool, sample_dataframe, sample_target):
    ...
    @patch('stock_market_analytics.modeling.model_factory.estimation.estimation_functions.Pool')
    def test_create_catboost_pool_no_categorical_features(self, mock_pool, sample_target):
...
class TestEstimationFunctionsIntegration:
    ...
    def test_pipeline_detect_standardize_pool(self):
        ...
        pool = create_catboost_pool(df, target)  # This internally calls standardize and detect
