...
class TestPrepareTrainingData:
    ...
    def test_prepare_training_data_default_params(self, mock_create_datasets, mock_split):
    ...
    @patch('stock_market_analytics.modeling.modeling_steps.split_data')
    @patch('stock_market_analytics.modeling.modeling_steps.create_modeling_datasets')
    def test_prepare_training_data_custom_params(self, mock_create_datasets, mock_split):
...
class TestCreateModelingDatasets:
    ...
    def test_create_modeling_datasets_default_target(self):
        ...
        with patch('stock_market_analytics.modeling.modeling_steps.config') as mock_config:
...
class TestCreateFeatureNameMapping:
    ...
    def test_create_feature_name_mapping_with_transformations(self):
        ...
        mock_transformations.get_feature_names_out.return_value = ["pca0", "pca1", "pca2"]
...
class TestAnalyzeFeatureImportance:
    ...
    @patch('stock_market_analytics.modeling.modeling_steps._create_feature_name_mapping')
    @patch('stock_market_analytics.modeling.modeling_steps._format_feature_importance_dataframe')
...
class TestCreateCalibratedPipeline:
    ...
    def test_create_calibrated_pipeline(self, mock_calibrator_class):
        ...
        mock_calibrator_class.create_calibrated_pipeline.return_value = (
            mock_calibrated_pipeline, mock_calibrator
        )
...
class TestEvaluateCalibratedPredictions:
    ...
    def test_evaluate_calibrated_predictions(self, mock_config, mock_evaluator_class):
        ...
        mock_evaluator.evaluate_calibrated_predictions.return_value = mock_conformal_results
        mock_evaluator_class.return_value = mock_evaluator
        ...
        mock_base_pipeline = Mock()
        mock_raw_predictions = np.array([[0.05, 0.25, 0.5, 0.75, 0.95], 
                                       [0.15, 0.35, 0.6, 0.85, 0.99]])
...
class TestCreateAndEvaluateCalibratedModel:
    ...
    @patch('stock_market_analytics.modeling.modeling_steps.evaluate_calibrated_predictions')
    def test_create_and_evaluate_calibrated_model(self, mock_evaluate_cal, mock_create_cal):
        ...
        result = _create_and_evaluate_calibrated_model(mock_base_pipeline, modeling_datasets)
...
class TestCompleteTrainingWorkflow:
    ...
    @patch('stock_market_analytics.modeling.modeling_steps._create_and_evaluate_calibrated_model')
    @patch('stock_market_analytics.modeling.modeling_steps.train_baseline_models')
    @patch('stock_market_analytics.modeling.modeling_steps._train_and_evaluate_main_model')
    ...
    def test_complete_training_workflow(self, mock_load, mock_prepare, mock_train_main, 
                                       mock_train_baselines, mock_calibrated):
        ...
        mock_train_baselines.assert_called_once_with(mock_data_prep["modeling_datasets"])
