...
def interpolated_df(sorted_df: pl.DataFrame) -> pl.DataFrame:
    ...
    sorted_df = sorted_df.with_columns(
        pl.col("*").forward_fill().over("symbol")
    ).drop_nulls()
...
def basic_indicators_df(
    interpolated_df: pl.DataFrame, horizon: int, short_window: int, long_window: int
) -> pl.DataFrame:
    """
    ...
    dollar_volume_expr = (
        (pl.col("close") * pl.col("volume")).over("symbol").alias("dollar_volume")
    )
    ...
    short_log_returns_expr = (
        pl.col("close")
        .log()
        .shift(short_window)
        .over("symbol")
        .alias("short_log_returns")
    )
    long_log_returns_expr = (
        pl.col("close")
        .log()
        .shift(long_window)
        .over("symbol")
        .alias("long_log_returns")
    )

    log_returns_ratio_expr = (short_log_returns_expr - long_log_returns_expr).alias(
        "log_returns_ratio"
    )
    ...
    up = (
        pl.when(pl.col("log_returns_d") > 0)
        .then(pl.col("log_returns_d"))
        .otherwise(0.0)
    )
    dn = (
        pl.when(pl.col("log_returns_d") < 0)
    ...
    sharpe_ratio_proxy = (
        total_returns
        / (pl.col("log_returns_d").rolling_std(window).over("symbol") + 1e-8)
    ).alias("sharpe_ratio_proxy")

    return interpolated_df.with_columns(
        [rsi_ewm_expr, sortino_ratio, sharpe_ratio_proxy]
    )


def volatility_indicators_df(
    basic_indicators_df: pl.DataFrame, long_window: int, short_window: int
) -> pl.DataFrame:
    """
    ...
    return basic_indicators_df.with_columns(
        [long_vol_expr, short_vol_expr, vol_ratio_expr]
    ).select(["symbol", "date", "vol_ratio", "long_vol_ewm", "short_vol_ewm"])


def volatility_features_df(
    volatility_indicators_df: pl.DataFrame, long_window: int
) -> pl.DataFrame:
    long_vol_ewm_expr = pl.col("long_vol_ewm")
    vol_of_vol_expr = (
        pl.col("short_vol_ewm")
        .rolling_std(long_window)
        .over("symbol")
        .alias("vol_of_vol_ewm")
    )

    vol_expansion_expr = (
        long_vol_ewm_expr / (long_vol_ewm_expr.shift(long_window).over("symbol") + 1e-8)
    ).alias("vol_expansion")

    return volatility_indicators_df.with_columns(
        [vol_of_vol_expr, vol_expansion_expr]
    ).select(["symbol", "date", "vol_ratio", "vol_of_vol_ewm", "vol_expansion"])


def momentum_indicators_df(
    basic_indicators_df: pl.DataFrame, long_window: int, short_window: int
) -> pl.DataFrame:
    """
    ...
    long_momentum_expr = (
        pl.col("log_returns_d")
        .rolling_sum(long_window)
        .over("symbol")
        .alias("long_momentum")
    )
    short_momentum_expr = (
        pl.col("log_returns_d")
        .rolling_sum(short_window)
        .over("symbol")
        .alias("short_momentum")
    )
    long_short_momentum_expr = (long_momentum_expr - short_momentum_expr).alias(
        "long_short_momentum"
    )
    ...
    return basic_indicators_df.with_columns([long_short_momentum_expr, cmo]).select(
        ["symbol", "date", "long_short_momentum", "cmo"]
    )


def momentum_features_df(
    momentum_indicators_df: pl.DataFrame, volatility_indicators_df: pl.DataFrame
) -> pl.DataFrame:
    momentum_indicators_df = momentum_indicators_df.join(
        volatility_indicators_df, on=["symbol", "date"], how="inner"
    )

    long_short_momentum_expr = pl.col("long_short_momentum")
    long_vol_ewm_expr = pl.col("long_vol_ewm")

    risk_adj_momentum_expr = (
        long_short_momentum_expr / (long_vol_ewm_expr + 1e-8)
    ).alias("risk_adj_momentum")

    return momentum_indicators_df.with_columns([risk_adj_momentum_expr]).select(
        ["symbol", "date", "long_short_momentum", "cmo", "risk_adj_momentum"]
    )


def liquidity_indicators_df(
    basic_indicators_df: pl.DataFrame, short_window: int, long_window: int
) -> pl.DataFrame:
    """
    ...
    turnover_proxy = (pl.col("dollar_volume") / (avg_dollar_volume + 1e-12)).alias(
        "turnover_proxy"
    )

    return basic_indicators_df.with_columns([amihud_illiq_expr, turnover_proxy]).select(
        ["symbol", "date", "amihud_illiq", "turnover_proxy"]
    )
...
def statistical_indicators_df(
    ...
    kurtosis_ratio_expr = (long_kurtosis_expr / (short_kurtosis_expr + 1e-8)).alias(
        "kurtosis_ratio"
    )
    ...
    short_rolling_mean = (
        pl.col("log_returns_d").rolling_mean(short_window).over("symbol")
    )
    short_rolling_std = pl.col("log_returns_d").rolling_std(short_window).over("symbol")

    long_zscore_expr = (
        (pl.col("log_returns_d") - long_rolling_mean) / (long_rolling_std + 1e-8)
    ).alias("long_zscore")
    short_zscore_expr = (
        (pl.col("log_returns_d") - short_rolling_mean) / (short_rolling_std + 1e-8)
    ).alias("short_zscore")
    zscore_ratio_expr = (long_zscore_expr / (short_zscore_expr + 1e-8)).alias(
        "zscore_ratio"
    )
...
def statistical_features_df(
    statistical_indicators_df: pl.DataFrame, horizon: int
) -> pl.DataFrame:
    ...
    statistical_indicators_df = statistical_indicators_df.with_columns(
        [pl.col("r2").shift(horizon).over("symbol").alias("r2_shifted")]
    )

    autocorr_expr = pl.rolling_corr(
        a=pl.col("r2"), b=pl.col("r2_shifted"), window_size=horizon
    ).over("symbol")

    y_hat_expr = (
        pl.when(autocorr_expr.is_finite())
        .then(autocorr_expr.clip(-1.0, 1.0))
        .otherwise(0)
        .alias("autocorr_r2")
    )
    ...
    p90 = (
        pl.col("log_returns_d")
        .rolling_quantile(
            quantile=0.90, interpolation="nearest", window_size=horizon * 4
        )
        .over("symbol")
    )
    p10 = (
        pl.col("log_returns_d")
        .rolling_quantile(
            quantile=0.10, interpolation="nearest", window_size=horizon * 4
        )
        .over("symbol")
    )
    ...
    return (
        statistical_indicators_df.with_columns([y_hat_expr, iqr_vol_expr])
        .select(
            [
                "symbol",
                "date",
...
def ichimoku_indicators_df(
    basic_indicators_df: pl.DataFrame, ichimoku_params: dict
) -> pl.DataFrame:
    ...
    span_b_now = ((high.rolling_max(p3) + low.rolling_min(p3)) / 2).over(
        "symbol"
    )  # B (no shift)
    ...
    atr = (
        tr.rolling_mean(window_size=atr_n).over("symbol").alias("atr") + 1e-8
    )  # avoid div by zero

    # --- Distances / spreads (ATR-normalized) ---
    price_above_cloud_atr = ((close - cloud_top_now) / atr).alias(
        "price_above_cloud_atr"
    )
    price_below_cloud_atr = ((cloud_bot_now - close) / atr).alias(
        "price_below_cloud_atr"
    )
    tenkan_kijun_spread_atr = ((tenkan - kijun) / atr).alias("tenkan_kijun_spread_atr")
    cloud_thickness_atr = ((span_a_now - span_b_now).abs() / atr).alias(
        "cloud_thickness_atr"
    )
...
def ichimoku_features_df(
    ichimoku_indicators_df: pl.DataFrame, ichimoku_params: dict
) -> pl.DataFrame:
    """
    ...
    def _slope(column: str):
        ...
        return (
            pl.col(column) - pl.col(column).shift(slope_window).over("symbol")
        ) / slope_window
    ...
    between_cloud = (
        ((close >= cloud_bot_now) & (close <= cloud_top_now))
        .cast(pl.Int8)
        .alias("between_cloud")
    )
    ...
    features |= {
        ...
        "above_cloud_persist": above_cloud.rolling_mean(persist_window)
        .over("symbol")
        .alias("above_cloud_persist"),
        "below_cloud_persist": below_cloud.rolling_mean(persist_window)
        .over("symbol")
        .alias("below_cloud_persist"),
    }
    ...
    features |= {
        "tenkan_cross_up": (
            (tenkan > kijun)
            & (tenkan.shift(1).over("symbol") <= kijun.shift(1).over("symbol"))
        )
        .cast(pl.Int8)
        .alias("tenkan_cross_up"),
        "tenkan_cross_dn": (
            (tenkan < kijun)
            & (tenkan.shift(1).over("symbol") >= kijun.shift(1).over("symbol"))
        )
        .cast(pl.Int8)
        .alias("tenkan_cross_dn"),
        "price_break_up": (
            (close > cloud_top_now)
            & (close.shift(1).over("symbol") <= cloud_top_now.shift(1).over("symbol"))
        )
        .cast(pl.Int8)
        .alias("price_break_up"),
        "price_break_dn": (
            (close < cloud_bot_now)
            & (close.shift(1).over("symbol") >= cloud_bot_now.shift(1).over("symbol"))
        )
        .cast(pl.Int8)
        .alias("price_break_dn"),
    }
    ...
    ichimoku_indicators_df = ichimoku_indicators_df.with_columns(
        pl.col("span_diff").shift(1).over("symbol").alias("span_diff_shifted")
    )
    ...
    features |= {
        "twist_event": ((span_diff * span_diff_shifted) < 0)
        .cast(pl.Int8)
        .alias("twist_event"),
    }
    ...
    features |= {
        "twist_recent": twist_event.rolling_sum(persist_window)
        .over("symbol")
        .alias("twist_recent")
    }
    ...
    return (
        ichimoku_indicators_df.with_columns(features.values())
        .select(
            [
                "symbol",
                "date",
...
def df_features(
    ...
    basic_indicators_df = (
        basic_indicators_df.join(
            liquidity_indicators_df, on=["symbol", "date"], how="inner"
        )
        .join(statistical_features_df, on=["symbol", "date"], how="inner")
        .join(momentum_features_df, on=["symbol", "date"], how="inner")
        .join(volatility_features_df, on=["symbol", "date"], how="inner")
        .join(ichimoku_features_df, on=["symbol", "date"], how="inner")
    )
