...
app.layout = html.Div(className="app-container", children=[
    ...
    html.Div(className="header", children=[
        ...
        html.P("Visualizing multi-quantile predictions with calibrated intervals.")
    ]),
    ...
    html.Div(className="controls-container", children=[
        ...
        html.Button('Generate Forecast', id='submit-button', n_clicks=0, className='submit-button')
    ]),
    
    # Main content area
    dcc.Loading(id="loading-spinner", type="cube", color="#1f77b4", children=[
...
def update_dashboard(n_clicks, symbol): # type: ignore
    ...
    interval_width = latest_data['pred_high_quantile'] - latest_data['pred_low_quantile']
    
    kpi_cards = html.Div(className="kpi-container", children=[
        html.Div(className="kpi-card", children=[
            html.H3("Latest Actual Log-Return"),
            html.P(f"{latest_data[config.modeling.target]:.4f}", className="kpi-value")
        ]),
        html.Div(className="kpi-card", children=[
    ...
    fig_intervals.add_trace(go.Scatter(
        x=merged_df['date'], y=merged_df['pred_high_quantile'],
    ...
    fig_intervals.add_trace(go.Scatter(
        x=merged_df['date'], y=merged_df['pred_low_quantile'],
        mode='lines', line=dict(width=0),
        fill='tonexty', fillcolor='rgba(31, 119, 180, 0.2)',
        name='95% Prediction Interval'
    ))
    fig_intervals.add_trace(go.Scatter(
        x=merged_df['date'], y=merged_df[config.modeling.target],
        mode='lines',
    ...
    for i in range(num_quantiles // 2):
        ...
        fig_fan.add_trace(go.Scatter(
            x=df_tail['date'], y=df_tail[upper_q_col],
            mode='lines', line=dict(width=0),
            showlegend=False
        ))
        fig_fan.add_trace(go.Scatter(
            x=df_tail['date'], y=df_tail[lower_q_col],
            mode='lines', line=dict(width=0),
            fill='tonexty',
    ...
    # Add median line
    fig_fan.add_trace(go.Scatter(
        x=df_tail['date'], y=df_tail['pred_Q_50'],
