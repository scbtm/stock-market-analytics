...
class DataQualityRules(BaseModel):
    ...
    check_volume_non_negative: bool = Field(
        default=True,
        description="Ensure volume is non-negative"
    )
    ...
    check_required_columns: bool = Field(
        default=True,
        description="Ensure all required columns are present and non-null"
    )
...
class DataQualityResult(BaseModel):
    ...
    failed_checks: list[str] = Field(
        ...
        description="List of failed validation check names"
    )
    ...
    total_rows_checked: int = Field(
        default=0,
        description="Total number of rows that were validated"
    )

    rows_with_issues: int = Field(
        default=0,
        description="Number of rows that had quality issues"
    )
