...
TEST_TICKER_LIMIT = 10  # TODO: Remove this limit for production

# Required columns for tickers file
REQUIRED_TICKER_COLUMNS = ["Symbol", "Name", "Country", "IPO Year", "Sector", "Industry"]
...
class BatchCollectionFlow(FlowSpec):
    ...
    def _load_tickers(self, base_data_path: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # TODO: Remove this limit for production use
            print(f"⚠️ Limiting to {TEST_TICKER_LIMIT} tickers for testing")
            tickers_df = tickers_df[:TEST_TICKER_LIMIT]
    ...
    def _create_collection_plan(self, symbol: str, metadata: Optional[Dict[str, Any]]) -> Optional[Dict[str, str]]:
        """
    ...
    def collect_data(self) -> None:
        ...
        try:
            ...
            if processed_data is not None and processor.processing_successful:
                new_metadata["max_date_recorded"] = processed_data["date"].max().strftime("%Y-%m-%d")
    ...
    def _update_metadata(self, base_data_path: Path, metadata_updates: List[Dict[str, Any]]) -> None:
        ...
        final_metadata_df = (
            combined_df
            .sort_values(["symbol", "last_ingestion"], ascending=[True, True])
    ...
    def _update_historical_data(self, base_data_path: Path, collected_data: List[pl.DataFrame]) -> None:
        ...
        final_data = (
            combined_data
            .sort(["symbol", "date"], descending=[False, False])  # Sort ascending for better performance
