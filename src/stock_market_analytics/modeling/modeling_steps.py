...
def create_modeling_splits(
    X: pl.DataFrame, y: pl.Series, test_size: float = 0.2, validation_size: float = 0.1
...
def fit_quantile_model(
    ...
    # Initialize model
    model = CatBoostQuantileRegressor(quantiles=quantiles, **model_params)
...
def evaluate_model_performance(
    ...
    y_pred_point = y_pred_quantiles[
        :, config.modeling.quantile_indices["MID"]
    ]  # Median
    ...
    interval_metrics = quantile_evaluator.evaluate_intervals(
        y_true, y_pred_quantiles[:, low_idx], y_pred_quantiles[:, high_idx], alpha=0.1
    )
...
def run_walk_forward_validation(
    ...
    splitter = WalkForwardSplitter(
        initial_train_size=initial_train_size, test_size=test_size, step_size=step_size
    )

    results = []

    for fold, (X_train, X_test, y_train, y_test) in enumerate(
        splitter.split_generator(X, y)
    ):
        if fold >= max_folds:
            break
        ...
        # Fit model
        model = CatBoostQuantileRegressor(
            quantiles=config.modeling.quantiles, **model_params
        )
...
def create_baseline_models(
    X_train: pl.DataFrame, y_train: pl.Series, X_test: pl.DataFrame, y_test: pl.Series
    ...
    for name, baseline in baselines.items():
        ...
        results[name] = {"metrics": metrics, "model": baseline}
...
def save_predictions(
    ...
    predictions_df = pl.DataFrame(
        {f"q{int(q * 100)}": predictions[:, i] for i, q in enumerate(quantiles)}
    )
