...
def create_catboost_pool(
    ...
    try:
        ...
        raise ImportError("CatBoost is required for this function")
    
    # Convert to pandas for CatBoost compatibility
    X_pandas = X.to_pandas()
    y_pandas = y.to_pandas() if y is not None else None
    
    # Identify categorical feature indices
    cat_indices = []
    if categorical_features:
        cat_indices = [X_pandas.columns.get_loc(col) for col in categorical_features if col in X_pandas.columns]
